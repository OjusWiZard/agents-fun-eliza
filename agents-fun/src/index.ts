...
import { getTokenForProvider, loadCharacterFromArgs, fetchSafeAddress, getSecrets, ROOMS } from './config/index'
 ...
 */
function createAgent(character: Character, db: any, cache: any, token: string): AgentRuntime {
 ...
 * periodic checking.
 */
async function runAgentAutonomously(runtime: AgentRuntime, directClient: DirectClient) {
...
async function main() {
  try {
    ...
    const serverPort = await getAvailablePort(parseInt(settings.SERVER_PORT || '3000'))

    // Load and configure the character
    const character: Character = await loadCharacterFromArgs()

    // Retrieve the safe address and assign secrets
    const safeAddress = fetchSafeAddress()

    if (character.settings === undefined) {
      character.settings = {}
    }
